...
const MidiReader: React.FC = () => {
  ...
  useEffect(() => {
    const fetchMidiFile = async () => {
      try {
        const response = await fetch('/55Dive.mid');
  ...
  const extractNoteData = () => {
    ...
    midiData.tracks.forEach((track: any[], trackIndex: number) => {
      let currentInstrument = 'Instrument 0';
      const notes: string[] = [];

      track.forEach((event: any) => {
        if (event.type === 'programChange') {
          currentInstrument = `Instrument ${event.programNumber}`;
        } else if (event.type === 'noteOn' && event.velocity > 0) {
          notes.push(getNoteName(event.noteNumber));
        }
      });
