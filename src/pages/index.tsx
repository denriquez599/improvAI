...
export default function Home() {
  ...
  const [folder, setFolder] = useState<Song[]>([]);
  const [description, setDesciption] = useState<string>("I have no notes, have fun!");
  return (
    <div className='flex h-screen bg-gradient-to-b from-spotifyGrey to-blackground'>
      ...
      {page == "Home Page" && <HomePage setSong={setSong} setPage={setPage} setFolder={setFolder} setDescription={setDesciption} />}
      {page == "Improvise" && <Improv setSong={setSong} song={song} />}
      {page == "Learn To Play" && <LearnToPlay setSong={setSong} song={song} />}
      {page == "Lesson Plan" && <LessonPlan folder={folder} description={description} />}
      {/* <Footer /> */}
    </div>
  );
}
